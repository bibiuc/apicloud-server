...
var utils = {
  ...
  getProjectFiles: function(project) {
    return fs.readFileAsync(path.join(project, '.syncignore'))
      ...
      .then(function (ignores) {
        return globby([/^.*\/$/.test(project) ? (project + '**/*') : (project + '/**/*')].concat(ignores), {nodir: true, absolute: false, realpath: false});
      });
  },
  ...
  TimestampFilter: makeFilterClass({
    ...
    reset(appId) {
      if (appId) {
        if (this.__timestamps[appId]) {
          delete this.__timestamps[appId];
        }
      } else {
        this.__timestamps = {};
      }
    ...
    add: function (appId, cmd) {
      if (!this.__timestamps[appId]) {
        this.__timestamps[appId] = cmd.timestamp;
      }
    },
